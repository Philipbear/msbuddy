                ...
                if int(clean_formula_dict["H"]) >= loss_h:
                    ...
                    for key in clean_formula_dict:
                        if int(clean_formula_dict[key]) == 0:
                            break
                        new_formula = (
                            new_formula + key + str(int(clean_formula_dict[key]))
                        )
                        ...
                        if key != "C":
                            if int(clean_formula_dict[key]) == 1:
                                ...
                                new_formula = (
                                    new_formula
                                    + key
                                    + str(int(clean_formula_dict[key]))
                                )
                new_formula_df.loc[len(new_formula_df)] = [new_formula, times]
                ...
                for key in formula_dict:
                    if key != "C":
                        if int(formula_dict[key]) == 1:
                            ...
                            new_formula = (
                                new_formula + key + str(int(formula_dict[key]))
                            )
            new_formula_df.loc[len(new_formula_df)] = [new_formula, times]
            charged_drop_index_list.append(index)
...
def parse_mass_dbe():
    ...
    for file in glob.glob(
        str("/Users/tshyun/Desktop/2022.2.10/COOP/workterm1/pyBUDDY/db/split_ori")
        + "/*.csv"
    ):
        ...
        for formula, PubChem, npatlas, Coconut in tqdm(df.itertuples(index=False)):
            ...
            if not (dbe >= -5 and mass <= 2000):
                ...
                mass_element_df.loc[len(mass_element_df)] = [
                    formula,
                    mass,
                    array[0],
                    array[1],
        ...
        mass_element_df.to_csv(
            "/Users/tshyun/Desktop/2022.2.10/COOP/workterm1/pyBUDDY/db/split_ori/chunkoutput{}.csv".format(
                number
            ),
            index=False,
        )
...
def concat_csv():
    ...
    count = 0
    for file in glob.glob(
        str("/Users/tshyun/Desktop/2022.2.10/COOP/workterm1/pyBUDDY/db/split_output")
        + "/*.csv"
    ):
        df = pd.read_csv(file)
...
def modify():
    ...
    df = df.rename(
        columns={"mass_x": "mass", "PubChem_x": "PubChem", "npatlas": "NPAtlas"}
    )
