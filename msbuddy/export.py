        ...
        result_df_rows.append(
            {
                ...
                if individual_result["estimated_fdr"] is not None
                else "NA",
    ...
    if write_details:
        for mf in batch_data:
            ...
            for m, cf in enumerate(mf.candidate_formula_list):
                ...
                if mf.ms2_processed:
                    if cf.ms2_raw_explanation:
                        ...
                        ms2_explan_idx = ",".join(
                            [str(x) for x in cf.ms2_raw_explanation.idx_array]
                        )
                        ms2_explan_str = ",".join(
                            [
                                x.__str__()
                                for x in cf.ms2_raw_explanation.explanation_list
                            ]
                        )
                    else:
                        exp_ms2_peak = "0"
                ...
                # theoretical mass
                theo_mass = (
                    cf.formula.mass * mf.adduct.m
                    + mf.adduct.net_formula.mass
                    - mf.adduct.charge * 0.0005486
                ) / abs(mf.adduct.charge)
                mz_error_ppm = (mf.mz - theo_mass) / theo_mass * 1e6
                all_candidates_df_rows.append(
                    {
                        "rank": str(m + 1),
                        "formula": cf.formula.__str__(),
                        "ms1_isotope_similarity": round(cf.ms1_isotope_similarity, 5)
                        if cf.ms1_isotope_similarity is not None
                        else "NA",
                        "mz_error_ppm": round(mz_error_ppm, 5),
                        "explained_ms2_peak": exp_ms2_peak,
                        "total_valid_ms2_peak": len(mf.ms2_processed)
                        if mf.ms2_processed
                        else "NA",
                        "estimated_prob": cf.estimated_prob
                        if cf.estimated_prob is not None
                        else "NA",
                        "normalized_estimated_prob": cf.normed_estimated_prob
                        if cf.normed_estimated_prob is not None
                        else "NA",
                        "estimated_fdr": cf.estimated_fdr
                        if cf.estimated_fdr is not None
                        else "NA",
            ...
            all_candidates_df.to_csv(
                mf_path / "formula_results.tsv", sep="\t", index=False
            )
