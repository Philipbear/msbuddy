...
def init_db() -> dict:
    ...
    basic_db, halogen_db = check_download_joblibload(
        "https://github.com/Philipbear/msbuddy/releases/download/msbuddy_data_v0.2.4/formula_db_v0.2.4.joblib",
        data_path / db_name,
    )

    (
        global_dict["basic_db_mass"],
        global_dict["basic_db_formula"],
        global_dict["basic_db_idx"],
    ) = basic_db
    (
        global_dict["halogen_db_mass"],
        global_dict["halogen_db_formula"],
        global_dict["halogen_db_idx"],
    ) = halogen_db
...
def init_ml_models(global_dict) -> dict:
    ...
    (
        global_dict["model_ms1_ms2"],
        global_dict["model_noms1_ms2"],
        global_dict["model_ms1_noms2"],
        global_dict["model_noms1_noms2"],
        global_dict["platt_a_0"],
        global_dict["platt_b_0"],
        global_dict["platt_a_1"],
        global_dict["platt_b_1"],
        global_dict["platt_a_2"],
        global_dict["platt_b_2"],
...
def _load_usi(usi: str, adduct: Union[str, None] = None) -> MetaFeature:
    ...
    if charge == 0:
        if adduct is not None:
            ...
            logging.warning(
                "Charge is 0 and adduct is not provided. Default charge +1 is used."
            )
...
def load_usi(
    usi_list: Union[str, List[str]], adduct_list: Union[None, str, List[str]] = None
) -> List[MetaFeature]:
    """
    ...
    if adduct_list is None:
        ...
        logging.warning(
            "adduct_list and usi_list must have the same length. Default adducts are used."
        )
    ...
    for idx, item in enumerate(usi_list):
        if item not in seen:
            ...
            logging.warning(
                "Duplicate USI: " + item + ". Only the first occurrence is used."
            )
