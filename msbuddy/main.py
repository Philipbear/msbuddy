...
class MsbuddyConfig:
    ...
    def __init__(
        ...
        isotope_bin_mztol: float = 0.02,
        max_isotope_cnt: int = 4,
        ...
        if ms_instr is None or ms_instr == "None":
            ...
            raise ValueError(
                "Invalid MS instrument. Please choose from 'orbitrap', 'fticr', 'qtof' and None."
            )
        ...
        if isotope_bin_mztol <= 0:
            ...
            logging.warning(
                f"Isotope bin m/z tolerance is set to {self.isotope_bin_mztol}."
            )
        else:
        ...
        if rel_int_denoise_cutoff < 0 or rel_int_denoise_cutoff >= 1:
            ...
            logging.warning(
                f"Relative intensity denoise cutoff is set to {self.rel_int_denoise_cutoff}."
            )
        else:
...
class Msbuddy:
    ...
    def _preprocess_and_generate_candidate_formula(
        self, batch_start_idx: int = 0, batch_end_idx: int = None
    ):
        """
        ...
        def _preprocess_and_gen_cand_nonparallel(
            meta_feature: MetaFeature, ps: MsbuddyConfig
        ) -> MetaFeature:
            """
    ...
    def annotate_formula_cmd(
        self, output_path: pathlib.Path, write_details: bool = False
    ):
        """
        ...
        for n in range(n_batch):
            ...
            result_summary_df = write_batch_results_cmd(
                self.data, output_path, write_details, start_idx, end_idx
            )
            result_summary_df_all = pd.concat(
                [result_summary_df_all, result_summary_df], ignore_index=True, axis=0
            )
        ...
        result_summary_df_all.to_csv(
            output_path / "msbuddy_result_summary.tsv", sep="\t", index=False
        )
    ...
    def _annotate_formula_prepare(self) -> int:
        ...
        if cnt_pre != cnt_post:
            tqdm.write(
                f"{cnt_pre - cnt_post} spectra with precursor mz > 1500 are removed."
            )

        if not self.data:
            raise ValueError("No data loaded.")
        query_str = (
            f"{len(self.data)} queries loaded."
            if len(self.data) > 1
            else "1 query loaded."
        )
        ...
        batch_str = (
            f"{n_batch} batches in total." if n_batch > 1 else "1 batch in total."
        )
    ...
    def _annotate_formula_main_batch(self, n: int, n_batch: int) -> Tuple[int, int]:
        ...
        predict_formula_probability(
            self.data, start_idx, end_idx, self.config, shared_data_dict
        )
    ...
    def mass_to_formula(
        ...
        if integer_dbe:
            out = [
                FormulaResult(form_arr_to_str(f.array), f.mass, mass)
                for f in formulas
    ...
    def mz_to_formula(
        ...
        if halogen:
            formulas, _ = query_precursor_mass(
                mz, ion, mz_tol, ppm, 1, shared_data_dict
            )
        else:
            formulas, _ = query_precursor_mass(
                mz, ion, mz_tol, ppm, 0, shared_data_dict
            )
        ...
        if integer_dbe:
            out = [
                FormulaResult(
                    form_arr_to_str(f.array),
                    (f.mass * ion.m + ion.net_formula.mass - ion_int * 0.00054858)
                    / abs(ion.charge),
                    mz,
                )
                for f in formulas
...
def _preprocess_and_gen_cand_parallel(
    meta_feature: MetaFeature, ps: MsbuddyConfig
) -> MetaFeature:
    """
    a wrapper function for data preprocessing and candidate formula space generation
...
def _gen_subformula(mf: MetaFeature, ps: MsbuddyConfig) -> MetaFeature:
    ...
    if min(cand_form_exp_ms2_peak_list) > 0:
        mf.candidate_formula_list = [
            cf for cf in mf.candidate_formula_list if len(cf.ms2_raw_explanation) > 0
        ]

    return mf


def _generate_candidate_formula(
    mf: MetaFeature, ps: MsbuddyConfig, global_dict
) -> MetaFeature:
    """
    preprocess data and generate candidate formula space
    :param mf: MetaFeature object
    ...
    mf.data_preprocess(
        ...
        ps.ms1_tol,
        ps.ms2_tol,
        ps.isotope_bin_mztol,
        ps.max_isotope_cnt,
        ps.rel_int_denoise_cutoff,
        ps.top_n_per_50_da,
    )
