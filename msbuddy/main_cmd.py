...
def main():
    parser = argparse.ArgumentParser(
        description="msbuddy command line interface (version 0.3.14)"
    )
    ...
    parser.add_argument(
        "-ms1_tol", type=float, default=5, help="MS1 tolerance. Default: 5."
    )
    parser.add_argument(
        "-ms2_tol", type=float, default=10, help="MS2 tolerance. Default: 10."
    )
    parser.add_argument(
        "-halogen",
        "-hal",
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Number of CPUs to use. Default: -1, use all CPUs.",
    )
    parser.add_argument(
        "-timeout_secs",
        "-t",
    ...
    parser.add_argument(
        "-c_min", type=int, default=0, help="Minimum number of C atoms. Default: 0."
    )
    parser.add_argument(
        "-c_max", type=int, default=80, help="Maximum number of C atoms. Default: 80."
    )
    parser.add_argument(
        "-h_min", type=int, default=0, help="Minimum number of H atoms. Default: 0."
    )
    parser.add_argument(
        "-h_max", type=int, default=150, help="Maximum number of H atoms. Default: 150."
    )
    parser.add_argument(
        "-n_min", type=int, default=0, help="Minimum number of N atoms. Default: 0."
    )
    parser.add_argument(
        "-n_max", type=int, default=20, help="Maximum number of N atoms. Default: 20."
    )
    parser.add_argument(
        "-o_min", type=int, default=0, help="Minimum number of O atoms. Default: 0."
    )
    parser.add_argument(
        "-o_max", type=int, default=30, help="Maximum number of O atoms. Default: 30."
    )
    parser.add_argument(
        "-p_min", type=int, default=0, help="Minimum number of P atoms. Default: 0."
    )
    parser.add_argument(
        "-p_max", type=int, default=10, help="Maximum number of P atoms. Default: 10."
    )
    parser.add_argument(
        "-s_min", type=int, default=0, help="Minimum number of S atoms. Default: 0."
    )
    parser.add_argument(
        "-s_max", type=int, default=15, help="Maximum number of S atoms. Default: 15."
    )
    parser.add_argument(
        "-f_min", type=int, default=0, help="Minimum number of F atoms. Default: 0."
    )
    parser.add_argument(
        "-f_max", type=int, default=20, help="Maximum number of F atoms. Default: 20."
    )
    parser.add_argument(
        "-cl_min", type=int, default=0, help="Minimum number of Cl atoms. Default: 0."
    )
    parser.add_argument(
        "-cl_max", type=int, default=15, help="Maximum number of Cl atoms. Default: 15."
    )
    parser.add_argument(
        "-br_min", type=int, default=0, help="Minimum number of Br atoms. Default: 0."
    )
    parser.add_argument(
        "-br_max", type=int, default=10, help="Maximum number of Br atoms. Default: 10."
    )
    parser.add_argument(
        "-i_min", type=int, default=0, help="Minimum number of I atoms. Default: 0."
    )
    parser.add_argument(
        "-i_max", type=int, default=10, help="Maximum number of I atoms. Default: 10."
    )
    parser.add_argument(
        "-isotope_bin_mztol",
        type=float,
    ...
    msb_config = MsbuddyConfig(
        ...
        o_range=(args.o_min, args.o_max),
        p_range=(args.p_min, args.p_max),
        s_range=(args.s_min, args.s_max),
        f_range=(args.f_min, args.f_max),
        cl_range=(args.cl_min, args.cl_max),
        br_range=(args.br_min, args.br_max),
        i_range=(args.i_min, args.i_max),
        isotope_bin_mztol=args.isotope_bin_mztol,
        max_isotope_cnt=args.max_isotope_cnt,
        rel_int_denoise_cutoff=args.rel_int_denoise_cutoff,
        top_n_per_50_da=args.top_n_per_50_da,
    )
    ...
    if args.output:
        ...
        output_path = (
            pathlib.Path(args.mgf if args.mgf else args.csv).parent / "msbuddy_output"
        )
    else:
    ...
    if args.mgf:
        ...
        if df.shape[1] > 1:
            engine.load_usi(
                usi_list=df.iloc[:, 0].tolist(), adduct_list=df.iloc[:, 1].tolist()
            )
        else:
